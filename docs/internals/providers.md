...
A provider is the agent runtime that does the actual work. ML Code supports several, and the
...
ML Code does not own an external OpenCode process. Native configuration changes there can require
an external reload or restart before ML Code's next refresh sees them.
