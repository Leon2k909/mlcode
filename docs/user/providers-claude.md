...
This guide is for people who want to use more than one Claude setup in ML Code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install ML Code](./install.md).
...
In ML Code Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means ML Code uses Claude Code's normal config directory.

When you set this field, ML Code points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, ML Code
...
ML Code looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where ML Code looks.

Then add another Claude provider in ML Code:
...
ML Code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so ML Code keeps separate config directories isolated
...
Add or edit a Claude provider in ML Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. ML Code stores the value as a server secret and does not
...
ML Code does not need a special Claude Code Router provider. Treat the router as a Claude
