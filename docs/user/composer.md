...
Messages can contain up to 120,000 characters. If a draft is longer, ML Code keeps it in the
...
also reports that skill as a native slash command, ML Code hides the duplicate native entry and keeps
...
start it in the background. ML Code opens another new thread and shows an **Open** action for the
