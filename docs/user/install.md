...
ML Code is a web and desktop GUI for running coding agents on your machine.

## Requirements

Node.js `^22.16 || ^23.11 || >=24.10` on the machine that runs the ML Code server.

At least one provider runtime, installed and authenticated. You can install Antigravity from
ML Code settings. See [Providers](#providers) below.
...
This starts the ML Code server on your machine and opens the local web app. Use
`npx t3@latest --help` for the full CLI reference.

## Open a project in the desktop app

When the ML Code desktop app is running on the same machine, open the current directory with:
...
thread. It does not launch the desktop app, open a browser, or start a ML Code server. A background
...
[GitHub Releases](https://github.com/Leon2k909/mlcode/releases), or install from a package
registry.
...
successful launch, ML Code keeps the current runtime and one previous runtime for rollback and
removes older caches automatically. If a cached runtime stops working, ML Code launches from the
application files under `/mnt/c` instead and reinstalls the runtime on the next launch.

## Providers

ML Code drives provider CLIs; it does not ship them. Install the CLI for each provider you want
to use and authenticate it, or use ML Code's managed setup for Antigravity.
...
| Antigravity | [Official ACP agent](https://github.com/agentclientprotocol/registry/blob/main/antigravity-acp/agent.json) | Managed by ML Code | **Sign in with Google** in ML Code |
...
ML Code looks for, but authenticate with `agent login`, not `cursor-agent login`.
...
Run CLI login commands on the machine running the ML Code server, not on the device you browse
from. Antigravity uses its sign-in controls in ML Code instead of a CLI login command.
...
ML Code. You can install ML Code, open it, and add providers afterwards. A provider that is not
authenticated shows its status and setup instructions in **Settings**, and fails at session start
...
- [Permission modes](./permission-modes.md): how much ML Code asks before acting
- [Remote access](./remote-access.md): connect from a phone, tablet, or another desktop
- [Keeping ML Code in sync](./updating.md): client and server version skew
