...
This guide is for people who want to use more than one Codex account in ML Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install ML Code](./install.md).
...
issue. ML Code uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
and share with OpenAI employees.

## Approve access to other apps

When a Codex tool needs access to an app such as Safari, ML Code shows the app name and asks for
...
In ML Code Settings, name it something obvious:
...
In ML Code Settings, add another Codex provider:
...
ML Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, ML Code treats it
