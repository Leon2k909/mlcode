  ...
  it.effect("bootstraps all projection states and writes projection rows", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(unsettledRows, [
        { settledOverride: "active", settledAt: null, unsettledAt: "2026-01-01T00:00:02.000Z" },
      ]);
      ...
      const deletedMessageRows = yield* sql<{
        ...
        WHERE thread_id = 'thread-1'
      `;
      // Deleting the user message leaves the assistant reply behind.
      assert.deepEqual(deletedMessageRows, [{ messageId: "message-2" }]);
    }),
  );
});
