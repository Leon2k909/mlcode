      ...
      switch (event.type) {
        ...
        case "thread.message-sent": {
          ...
          yield* projectionThreadRepository.upsert({
            ...
            // An employee handoff dispatches a role:"user" message carrying an
            // employeeId — agent-authored, not typed by the human. Counting it
            // would reset this timestamp on every handoff (mirrors the guard on
            // the full-refresh path above).
            latestUserMessageAt:
        ...
        // A deletion can lower latestUserMessageAt, so it still needs the full
        // summary refresh below rather than the monotonic fast path above.
