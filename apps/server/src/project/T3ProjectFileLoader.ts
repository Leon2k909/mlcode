/**
 * T3ProjectFileLoader - Effect service that loads the checked-in `ml.json`
...
/** Service tag for ml.json project file loading. */
export class T3ProjectFileLoader extends Context.Service<
  T3ProjectFileLoader,
  {
    /**
     * Load and decode `ml.json` at the workspace root. Existing `t3.json`
     * checkouts are accepted as a migration fallback.
     *
...
export const make = Effect.gen(function* () {
  ...
  const readRaw = Effect.fn("T3ProjectFileLoader.readRaw")(function* (
    ...
    return Option.map(raw, (contents) => ({ filePath, contents }));
  });

  const load: T3ProjectFileLoader["Service"]["load"] = Effect.fn("T3ProjectFileLoader.load")(
    function* (workspaceRoot) {
      const raw = yield* readRaw(workspaceRoot, T3_PROJECT_FILE_NAME).pipe(
        Effect.flatMap((primary) =>
          Option.isSome(primary)
            ? Effect.succeed(primary)
            : readRaw(workspaceRoot, LEGACY_T3_PROJECT_FILE_NAME),
        ),
      );
      if (Option.isNone(raw)) {
        return Option.none<T3ProjectFile>();
      }
      return yield* decodeT3ProjectFileJson(raw.value.contents).pipe(
        Effect.map(Option.some),
        Effect.catchTags({
          SchemaError: (error) =>
            logT3ProjectFileLoadError(
