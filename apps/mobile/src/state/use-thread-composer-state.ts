...
// the provider's showInteractionModeToggle) so the composer keeps compiling
// without reintroducing legacy plan mode.
...
export function useThreadComposerState() {
  ...
  const onSendMessage = useCallback(async () => {
    ...
    // NOTE: fork keeps its thread-goal command handling. Upstream's Codex
    // in-composer feedback path (parseCodexFeedbackCommand/submitCodexFeedback/
    // uploadThreadFeedback) was dropped here — those helpers do not exist in
    // this fork. The Antigravity model-unavailable guard above is kept.
