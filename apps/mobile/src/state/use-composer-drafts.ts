...
/**
 * Copies a source draft's text and attachments into the target draft, but only
 * when the target is still empty. Persisted drafts hydrate first so a saved
 * target is never clobbered by the source just because it had not loaded into
 * memory yet (e.g. copying a new-task draft between projects).
 */
export async function copyComposerDraftContentIfEmpty(
  ...
  const current = appAtomRegistry.get(composerDraftsAtom);
  const target = normalizeDraft(current[targetKey]);
  const source = normalizeDraft(current[sourceKey]);
  if (!isEmptyDraft(target) || isEmptyDraft(source)) {
    return;
  }
  if (persistTimer !== null) {
    clearTimeout(persistTimer);
