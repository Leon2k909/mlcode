...
/**
 * The pill / card container — renders native glass via GlassSurface on
 * supported iOS 26+ devices (progressive blur, native morph), opaque card
 * otherwise. Exported so NewTaskDraftScreen can render the same composer chrome.
 */
...
export const ThreadComposer = memo(function ThreadComposer(props: ThreadComposerProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      armAgentAwarenessLiveActivityForLocalWork({
        ...
        projectTitle: props.environmentLabel ?? "ML Code",
      });
    } finally {
  ...
  // Preserve the fork's employee routing when the model changes on the same
  // provider instance.
  ...
  // NOTE: Merge resolution — upstream reworked existing-thread settings into a
  // native route, but that route is not wired into the fork's navigation (no
  // ExistingThreadSettingsRouteProvider is mounted and no screen is registered),
  // so using it here would crash. The fork keeps the modal ThreadSettingsSheet
  // (rendered below), the surface actually mounted in this app, which carries the
  // employee composer. openSettings dismisses the keyboard and opens that sheet.
  const openSettings = settingsSheetPresentation.open;

  // Keep the fork's active-employee label visible on the composer's inline
  // model control (personas / group chats), prefixed before the model name.
  const composerModelLabel = currentModelOption?.label ?? currentModelSelection.model;
  const employeeLabel = employeeSelectionLabel(
    props.serverConfig?.settings.employees,
    currentModelSelection,
  );
  const composerSettingsLabel = employeeLabel
    ? `${employeeLabel} · ${composerModelLabel}`
    : composerModelLabel;
      ...
      <ThreadSettingsSheet
        ...
        onUpdateEmployeeSelection={(employeeId, employeeIds) => {
          ...
          if (nextSelection !== null) {
            onUpdateModelSelection(nextSelection);
          }
        }}
