...
function useMarkdownStyles(onLinkPress: (href: string) => void): MarkdownStyleSets {
  ...
  const markdownTheme = useUniwindTheme();
  const iconSubtleColor = markdownTheme["--color-icon-subtle"];
  const inlineSkillForeground = markdownTheme["--color-inline-skill-foreground"];
  const userBubbleForegroundMuted = markdownTheme["--color-user-bubble-foreground-muted"];
  ...
  return useMemo(() => {
    ...
    const createMarkdownRenderers = (
      ...
      // NOTE: upstream's Markdown-fallback `image` renderer is dropped here —
      // this fork keeps useMarkdownStyles single-arg, so `renderImage` is not in
      // scope. Inline images still render through the native
      // SelectableMarkdownText path (AssistantMarkdownContent / UserMessageContent
      // pass renderImage to it directly).
      code_inline: ({ content }) => (
        <MarkdownInlineCode
          content={content ?? ""}
...
function renderFeedEntry(
  ...
  props: Pick<
    ThreadFeedProps,
    "employees" | "environmentId" | "onUseArtifactTemplate" | "skills"
  > & {
  ...
  if (entry.type === "message") {
    ...
    // NOTE: merge — upstream renders assistant citations as text; the fork strips
    // <handoff to="..."> tags. Compose both by deriving the handoff display from
    // the citation-rendered text, so non-employee messages keep upstream behavior
    // (handoffDisplay.visibleText === renderedText when there is no employee).
    const renderedText = renderAssistantCitationsAsText(message.text);
    const handoffDisplay =
      message.employeeId !== undefined
        ? deriveEmployeeHandoffDisplay(renderedText)
        : { visibleText: renderedText, toEmployeeId: undefined };
    ...
    return (
      <Animated.View
        className={cn(showAssistantMeta || isEmployeeUser ? "mb-5 px-1" : "mb-1 px-1")}
...
export const ThreadFeed = memo(function ThreadFeed(props: ThreadFeedProps) {
  ...
  // NOTE: this fork's useMarkdownStyles takes only the link handler — its markdown
  // colors come from MARKDOWN_COLORS, not a renderImage renderer. The image
  // renderers above are still used by renderItem / UserMessageContent /
  // AssistantMarkdownContent through the native SelectableMarkdownText path.
