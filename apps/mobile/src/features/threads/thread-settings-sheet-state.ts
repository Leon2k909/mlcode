...
// NOTE: reconstructed on merge — upstream extracted this catalog-search helper
// into this module (the test and ThreadSettingsSheet import it here) but the
// definition was lost in the merge. Behaviour matches the test contract:
// blank/whitespace query matches everything, otherwise a case-insensitive
// substring match against the model label, subtitle, and provider label.
export function modelMatchesCatalogQuery(input: {
  ...
  return [input.model.label, input.model.subtitle, input.providerLabel].some((value) =>
