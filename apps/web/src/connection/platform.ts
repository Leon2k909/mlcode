...
function clientMetadata() {
  // Adopt upstream's richer presentation metadata (browser/device detection,
  // hosted vs. server deployment), but keep our fork's "ML Code" branding on
  // the user-facing device label — the shared helper hardcodes "T3 Code".
  ...
  return {
    ...metadata,
    label: metadata.surface === "desktop" ? "ML Code Desktop" : "ML Code Web",
  };
}
