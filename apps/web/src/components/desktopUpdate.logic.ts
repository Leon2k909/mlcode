...
// Our fork publishes its own releases, so both the history and tag links point
// at Leon2k909/mlcode rather than upstream's pingdotgg/t3code repo.
const DESKTOP_RELEASE_HISTORY_URL = "https://github.com/Leon2k909/mlcode/releases";
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  if (action === "download") {
    return "This Mac has Apple Silicon, but ML Code is still running the Intel build under Rosetta. Download the available update to switch to the native Apple Silicon build.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but ML Code is still running the Intel build under Rosetta. Restart to install the downloaded Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but ML Code is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateButtonTooltip(state: DesktopUpdateState): string {
  ...
  if (state.status === "downloaded") {
    return `Update ${state.downloadedVersion ?? state.availableVersion ?? "ready"} downloaded. Press again to restart and install.`;
  }
  if (state.status === "error") {
    if (state.errorContext === "download" && state.availableVersion) {
...
export function getDesktopUpdateInstallConfirmationMessage(
  ...
  const windowsInstallWarning = isWindowsPlatform(platform)
    ? "\n\nOn Windows, ML Code will close and show an installer with progress. It will reopen automatically when installation finishes."
    : "";
  return `Install update${version ? ` ${version}` : ""} and restart ML Code?\n\nAny running tasks will be interrupted. Make sure you're ready before continuing.${windowsInstallWarning}`;
}
