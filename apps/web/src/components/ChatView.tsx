...
type ChatViewProps =
  | {
      ...
      workspaceHeaderControls?: ReactNode;
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const composerRef = workspaceActive ? (sharedComposerRef ?? localComposerRef) : localComposerRef;
  ...
  const shouldUseRightPanelSheet = workspaceMultiPane || viewportUsesRightPanelSheet;
  ...
  const visibleRightPanelOpen = workspaceActive && rightPanelOpen;
  ...
  }, [togglePreviewPanel, workspaceActive]);
  ...
  useEffect(() => {
    if (!workspaceActive || !activeThread?.id || terminalUiState.terminalOpen) return;
  ...
  }, [activeThread?.id, focusComposer, terminalUiState.terminalOpen, workspaceActive]);
  ...
  useEffect(() => {
    ...
    terminalUiOpenByThreadRef.current[activeThreadKey] = current;
  }, [activeThreadKey, focusComposer, terminalUiState.terminalOpen, workspaceActive]);
  ...
  const onSend = async (
    ...
    // Interaction-mode slash commands (/plan, /default) only act when enabled;
    // otherwise they send as plain text. Providers without the legacy toggle
    // receive their native commands unchanged.
    ...
    if (!queueLocally && isDraftHeroState && activeThreadKey) {
    ...
    // NOTE: The fork's session outbox type (QueuedThreadOutboxAttachment in
    // threadOutbox.ts) is image-only, and this list feeds the local-queue payload
    // below, so optimistic attachments stay image-only here. Files still send via
    // turnAttachmentsPromise; only their pre-confirmation preview waits for the
    // server echo. Upstream's inline anchoring/optimistic-insert/toast/clear block
    // was dropped because the fork's queueLocally/else branch below already does it.
  ...
  const dispatchQueuedMessage = useCallback(
    async (queued: QueuedThreadOutboxMessage, mode?: ThreadTurnDispatchMode): Promise<boolean> => {
      ...
      setThreadError(payload.threadId, null);

      const turnAttachmentsResult = await settlePromise(() => payload.attachmentsPromise);
      if (turnAttachmentsResult._tag === "Failure") {
        if (!isAtomCommandInterrupted(turnAttachmentsResult)) {
          const error = squashAtomCommandFailure(turnAttachmentsResult);
          setThreadError(
            payload.threadId,
            error instanceof Error ? error.message : "Failed to prepare queued message.",
          );
        }
        sendInFlightRef.current = false;
        resetLocalDispatch();
      ...
      liveFollowUserScrollGenerationRef.current = anchorUserScrollGenerationRef.current;
      setTimelineLiveFollowEnabled(true);
      pendingTimelineAnchorRef.current = payload.messageId;
      activeTimelineAnchorIndexRef.current = null;
      showScrollDebouncer.current.cancel();
      setShowScrollToBottom(false);
      setTimelineAnchor({
        threadKey: scopedThreadKey(scopeThreadRef(thread.environmentId, payload.threadId)),
      ...
      setOptimisticUserMessages((existing) => [
        ...existing,
        {
          ...
          role: "user",
          text: payload.text,
          ...(payload.optimisticAttachments.length > 0
      ...
      setOptimisticUserMessages((existing) => {
        return existing.filter((message) => message.id !== payload.messageId);
      });
      if (!isAtomCommandInterrupted(failure)) {
        const error = squashAtomCommandFailure(failure);
        setThreadError(
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's ML Code server to browse pull requests."
      />
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        >
          ...
          <ChatHeader
            ...
            // NOTE: The fork's `changeRequest={activeThreadChangeRequest}` prop was
            // dropped here — the merged ChatHeader (chat/ChatHeader.tsx) has no
            // `changeRequest` prop and `activeThreadChangeRequest` is undefined in this
            // file, so upstream's ChatHeader refactor removed that fork wiring. The
            // fork's displayProjectName() wrapping is kept. Re-add both if ChatHeader
            // regains the prop and the change-request snapshot source is restored.
            activeProjectName={
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            ...
            {workspaceActive && activeThreadRef && activePreviewMiniPlayer ? (
      ...
      {workspaceActive && rightPanelPresent && !shouldUseRightPanelSheet && activeThreadRef ? (
        <RightPanelTabs
          mode="inline"
          open={rightPanelOpen}
          maximized={rightPanelMaximized}
