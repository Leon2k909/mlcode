...
import { CodexRealtimeVoiceControl } from "./CodexRealtimeVoiceControl";
...
const ComposerFooterPrimaryActions = memo(function ComposerFooterPrimaryActions(props: {
  ...
  return (
    <>
      {(props.showSecondaryStatus ?? true) && props.activeContextWindow ? (
        <ContextWindowMeter
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  // NOTE: upstream's `resolvedCompactDisabledReason` was dropped — it referenced an
  // undefined `compactDisabledReason` and was only consumed by upstream's
  // ContextWindowMeter onCompact path, which the fork's meter (kept above) does not
  // use (compaction runs through the ChatComposerHandle.compactContext instead).
  ...
  // Plan mode is a legacy feature behind Settings → Beta. Upstream's resolver
  // computes the effective interaction mode and whether the toggle should show;
  // the fork keeps its provider-toggle memo (consumed by the composer controls).
  ...
  const submitCitationAndSend = useCallback(() => {
    // Cite the selected assistant text, then send as a normal turn — the
    // queue/steer axis is handled by submitComposer's dispatch mode.
    submitComposer();
  }, [submitComposer]);
  ...
  const onComposerPaste = (event: React.ClipboardEvent<HTMLElement>) => {
    ...
    event.preventDefault();
    void addComposerAttachments(candidateFiles);
  };
  ...
  // Render
  // ------------------------------------------------------------------
  // NOTE (merge): The fork's and upstream's composer render trees were
  // structurally irreconcilable at the line level (git kept the fork's render
  // as the "common" spine while upstream's full render lived inside the
  // conflict sides). Per the merge guide's fallback for irreconcilable
  // conflicts, the fork's render is kept intact so ALL fork features survive:
  // the local queue (QueuedMessagesPanel), prompt stash, context-window meter,
  // employee picker, file-drop drag handlers, and realtime voice control.
  // Upstream's non-render logic IS adopted above (attachment-upload effects,
  // provider resolver, slash-menu skills, tasks/banner derivations), so file
  // attachments still upload and send. What is NOT yet wired into this render:
  // upstream's banner-stack UI, resting/scroll-collapse controls, tasks
  // drawer/badge, attachment-upload progress + video previews, and the
  // editor's citation/page-scroll props. Several upstream derivations above
  // (bannerStackItems, restingBlockDefs, composerControls, inlineTasksBadge,
  // activityStackItem, collapsed image previews, etc.) are therefore currently
  // unused. Primary agent: integrate upstream's render UI or prune the unused
  // derivations.
  return (
    <form
      ref={composerFormRef}
