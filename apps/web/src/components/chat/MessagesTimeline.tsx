...
import { deriveEmployeeHandoffDisplay } from "../../employees";
...
const TimelineRowContent = memo(function TimelineRowContent({ row }: { row: TimelineRow }) {
  ...
  // NOTE: upstream's row refactor consolidated the tool-group expansion flags to a
  // single `isExpandedToolGroup`; the fork's per-entry `isExpandedToolGroupEntry`/
  // `isLastExpandedToolGroupEntry` row fields no longer exist, so those stale reads
  // were dropped. Employee-message detection above is preserved.
...
function EmployeeTimelineIdentity({
  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup>{hoverLabel}</TooltipPopup>
    </Tooltip>
  );
}
  ...
  return (
    <div className="group flex flex-col items-end gap-1">
      ...
      <div className="flex w-full max-w-[80%] items-center justify-end pe-1 text-xs tabular-nums opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover:opacity-100">
        <div className="flex shrink-0 items-center gap-2" data-user-message-actions="true">
          <Tooltip>
            <TooltipTrigger render={<p className="text-muted-foreground text-xs tabular-nums" />}>
              {formatDayAwareTimestamp(row.message.createdAt, ctx.timestampFormat)}
            </TooltipTrigger>
        ...
        {messageDisplay.text.length > 0 ? (
          ...
          >
            <ChatMarkdown
              ...
              lineBreaks={shouldPreserveAssistantLineBreaks(messageDisplay.text)}
              skills={ctx.skills}
...
const AgentSpawnCtaRow = memo(function AgentSpawnCtaRow(props: { workEntry: TimelineWorkEntry }) {
  ...
  return (
    <button
      ...
      aria-label="Temporary Codex/Claude provider subagents, not workspace employees"
