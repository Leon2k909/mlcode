...
function renderRunningActions(hasSendableContent = false, queuesRunningTurns = false) {
  return renderToStaticMarkup(
    createElement(ComposerPrimaryActions, {
      compact: true,
      pendingAction: null,
...
describe("ComposerPrimaryActions", () => {
  ...
  it("shows Queue as the primary action while a turn is running", () => {
    ...
    expect(markup).toContain('aria-label="Hold this message for the next turn"');
    expect(markup).toContain('aria-label="Steer now (Ctrl+Enter)"');
    expect(markup).toContain('aria-keyshortcuts="Control+Enter"');
    expect(markup.indexOf('aria-label="Hold this message for the next turn"')).toBeLessThan(
      markup.indexOf('aria-label="Steer now (Ctrl+Enter)"'),
    );
  });
  ...
  it("keeps steering as the hover action beside Queue", () => {
    ...
    expect(markup).toContain('aria-label="Queue this message for the next turn"');
    expect(markup).toContain('aria-label="Steer now (Ctrl+Enter)"');
